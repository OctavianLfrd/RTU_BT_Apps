...
final class Logger { // [lines: 3]
    ...
    static func print(_ level: LogLevel, message: String, file: String = #file, line: Int = #line) {
        ...
        // closure: [dd: 23]
        Task(priority: .background) { // [rd: { init level, init message, init file, init line } (4)]
            await lock.synchronize {
                defer {
...
extension Logger : Archivable {
    ...
    func getArchivableUrl() async -> URL? {
        // closure: [dd: 4]
        return await Logger.lock.synchronize { // [rd: { init Logger.lock } (1)]
            let logDirectoryUrl = Logger.getLogDirectoryUrl() // [rd: { init Logger } (1)]
            
            guard FileManager.default.fileExists(atPath: logDirectoryUrl.path) else { // [rd: { init FileManager.default, init logDirectoryUrl } (2)]
                return nil
            }
            
            return logDirectoryUrl // [rd: { init logDirectoryUrl } (1)]
        }
    }
} // [lines: 104]
